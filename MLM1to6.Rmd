...
in the`PLOT` command, Mplus will save 500 factor score values drawn for _each_ person separately. Factor scores include latent variables, but also random effects (i.e., means, slopes, or residual variances). These are drawn from the sampled parameter values of that person. Through using this option, we can then plot the means or medians of these per person (e.g., in histograms univariately, or in scatter plots bivariately). Furthermore, it will also allow us to ask for a comparison of the factor scores (i.e., the individual's parameter estimates), as we will show below.
...
As already mentioned, the `BITER = (3000)` statement can be used to ensure that Mplus draws at least 5000 samples in the MCMC algorithm; when the Potential Scale Reduction (PRS) statistic indicates that the algorithm did not converge yet, Mplus will continue beyond 5000 samples until the criterion is satisfied. If the user does not want additional samples, they should use `FBITER = 3000` instead. 

The PRS statistic is shown on the screen while running the analysis; this information is saved in the output by asking for `TECH8` in the `OUTPUT` command. For the current analysis, this is 
...
The PSR is based on computing for each model parameter: a) the total variance across both chains over the last 100 (retained) iterations; b) the variance within the two chains; and c) dividing the former by the latter. Hence, when two chains have settled in the same area of the parameter space, the total variance will be (almost) identical to the within chain variance, such that this statistic is very close to 1. The PSR that is reported here, is the largest one among all the model parameters; which parameter it is, is shown in the last column (which matches the parameter numbering in the `TECH1` output).

In addition to this statistic, the user should also check the trace plots of the model parameters. These can be viewed in Mplus, using the plotting option (from the plot menu, the option: Bayesian posterior parameter trace plot). An example of such a trace plot is given below.

![](images/Model0TracePlot.png){width=90%}

It shows a red and a blue chain which are based on the two different seeds (see `TECH8` output). In this case, these overlap well, which means that the total variance will be (almost) identical to the within chain variance. Furthermore, the plot shows no evidence for a trend (a trajectory going up or down), which would be considered a sign that the MCMC algorithm did not converge if it was present. Also, there are no extreme values, which would be some reason for concern about convergence (see the discussion of the results for Model 5 below). 
