...
Instead of analyzing the data for each person separately using replicated $N=1$ analyses, we can analyze the data of the 129 individuals in a single multilevel model. 

In the book chapter we consider 6 dynamic multilevel models: 

* __Multilevel models 1-3__ are based on combining _time series modeling_ with _multilevel modeling_

* __Multilevel models 4-6__ are based on combining _times series modeling_ and _multilevel modeling_ with _structural equation modeling_ 

As an initial step, we determine the intraclass correlation of each variable in __Multilevel Model 0__. When discussing this model, we will also provide some additional information regarding Bayesian estimation which apply more generally to the analyses performed for the book chapter. 
...
# General comments
Dynamic multilevel modeling is based on decomposing the observed variables into a within-person part and a between-person part, that is

$x_{it} = x_{it}^{(w)} + x_{i}^{(b)}$

$y_{it} = y_{it}^{(w)} + y_{i}^{(b)}$

For both components a model is specified: At the within level a time series model is used to account for the dynamics in the repeated measures within a person, and at the between level the random effects can be related to each other, either through correlating them, or by including them in a regression model, a path model, and/or a factor model. 

In Mplus, this approach requires the repeated measures within a person to be represented in long format (as was the case for the $N=1$ approach as well), with the data of all the individuals stacked. There has to be a specific variable that indicates to which individual the data belong (i.e., the clustering of the data). 

The `DATA` command now becomes:

      DATA: FILE = ChapterData1.dat;
...
in the`PLOT` command, Mplus will run the MCMC algorithm for another 500*100 iterations, and save every 100th draw of each factor score for _each person separately_. Note that factor scores include latent variables, but also random effects (i.e., means, slopes, and (residual) variances). Hence, this gives us information about the posterior distribution of each person separately. 

Through using this option, we can plot the means or medians of these per person (e.g., in histograms univariately, or in scatter plots bivariately). Furthermore, it will also allow us to ask for a comparison of the factor scores (i.e., the individual's parameter estimates), as we will show below.
...
[here](https://ellenhamaker.github.io/DSEM-book-chapter/mlm0.htm).
...
The part of the output that is of interest to us, are the last two lines where the intraclass correlations are computed for each variable. Note that by definition, the 95% CIs that are computed for variance related parameters (e.g., variances, residual variances, proportions of explained variance) will not cover zero; hence, when zero is not included in the CI of an intraclass correlation, this should not be interpreted as evidence that the intraclass correlation is different from zero. Instead, we may want to consider the lower bound and determine whether this is relatively close to zero or not, based also on the mean of the posterior distribution (i.e., the point estimate that is reported).
...
## Bayesian estimation
In generally, when using Bayesian estimation, we should be aware that a model may not have converged. While it is not possible to determine whether a model in fact did converged, there are several diagnostics that can be consulted to see whether there is evidence that the model did _not_ converge. 

As already mentioned, the `BITER = (3000)` statement can be used to ensure that Mplus draws at least 3000 samples in the MCMC algorithm; when the Potential Scale Reduction (PRS) statistic indicates that the algorithm did not converge yet, Mplus will continue beyond 3000 samples until the criterion is satisfied. If the user does not want additional samples, they should use `FBITER = 3000` instead. 

The PRS criterion is shown on the screen while running the analysis; this information is saved in the output by asking for `TECH8` in the `OUTPUT` command. For the current analysis, this is 
...
The PSR is based on computing _for each model parameter_ at each iteration: a) the total variance across both chains over all the iterations so far (i.e., both within and between chains); b) the variance within the two chains and averaging these; and c) dividing the former by the latter and taking the square root of this. Hence, when two chains have settled in the same area of the parameter space, the total variance will be (almost) identical to the within chain variance (or: between chain variance will be virtually zero), such that this criterion is very close to 1. The PSR that is reported here, is the largest one among all the model parameters; which parameter it is, is shown in the last column (which matches the parameter numbering in the `TECH1` output).

To compute the PSR, at least two chains are needed; this is the default number of chains used when doing Bayesian estimation in Mplus. The number can be changed in the `ANALYSIS` command, for instance by adding `CHAINS = 4;`. 

In addition to the PSR criterion, the user should also check the trace plots of the model parameters. These can be viewed in Mplus, using the plotting option (from the plot menu, the option: Bayesian posterior parameter trace plot). An example of such a trace plot is given below.


![](images/Model0TracePlot.png){width=90%}


It shows a red and a blue chain which are based on the two different seeds (see `TECH8` output). In this case, these overlap well, which means that the total variance will be (almost) identical to the within chain variance. Furthermore, the plot shows no evidence for a trend (a trajectory going up or down); if such a trend would be present, this would be considered a sign that the MCMC algorithm did not converge. Also, there are no extreme values, which would be some reason for concern about convergence (see the discussion of the results for Model 5 below). 

Other plots that contain relevant information on the performance of the MCMC algorithm are the autocorrelation plots that can be obtained for each model parameter based on each chain, as shown below. 


![](images/ParameterACF.png){width=90%}


Preferably, these autocorrelations (i.e., the autocorrelation across samples of the MCMC draws) go to zero very quickly, as is the case here. If they remain higher for longer lags, this is a reason to: a) using the `THINNING` option; and/or b) increase the number of iterations. 

As a final comment regarding Bayesian estimation, we point out that the prior distributions that were used for the model parameters, are also reported in the output at the end. For the current model these are
...
where the parameter numbers match the ones provided in the TECH1 output (based on the model matrices of the LISREL notation):
...
## Output MLM1
The output for this analysis is can be found 
[here](https://ellenhamaker.github.io/DSEM-book-chapter/mlm1.htm).
...
## Output MLM2
The output for this analysis is can be found 
[here](https://ellenhamaker.github.io/DSEM-book-chapter/mlm2.htm).
...
## Output MLM3
The output for this analysis is can be found 
[here](https://ellenhamaker.github.io/DSEM-book-chapter/mlm3.htm).
...
## Output MLM4
The output for this analysis is can be found 
[here](https://ellenhamaker.github.io/DSEM-book-chapter/mlm4.htm).
...
## Output MLM5
The output for this analysis is can be found 
[here](https://ellenhamaker.github.io/DSEM-book-chapter/mlm5_indirect.htm).
...
## Output MLM6
The output for this analysis is can be found 
[here](https://ellenhamaker.github.io/DSEM-book-chapter/mlm6_indirect.htm).
